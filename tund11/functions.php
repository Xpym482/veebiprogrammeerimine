<?php
  ...
  function readAllPublicPictureThumbsPage(){
	  ...
	  $stmt->bind_result($filenameFromDbm $altFromDb);
	  $stmt->execute();
	  if($stmt->fetch()){
  ...
  function latestPicture($privacy){
	  $html = "<p>Pole pilti, mida naidata";
	  $mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	  $stmt = $mysqli->prepare("SELECT filename, alttest FROM vpphotos3 WHERE id=(SELECT MAX(id) FROM vpphotos3 WHERE privacy=? AND deleted is NULL)");
	  echo $mysqli->error;
	  $stmt->bind_param("i", $privacy);
